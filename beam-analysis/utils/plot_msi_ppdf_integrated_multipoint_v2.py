...
def analyze_multiplexing_ambiguity(source_points_mm, base_dir, n_layouts, 
    ...
    for l_idx in range(n_layouts):
        ...
        # THE FIX: Now dynamically obeys MIN_MPXI
        mpxi_lookup = {
...
def visualize_map(data_2d, source_points_mm, out_dir, title_prefix, 
    ...
    # THE FIX: Finding the maximum NON-ZERO value
...
if __name__ == "__main__":
    # FILE PATHS
    DATA_DIR = "../../../data/hybrid_sc_spect_mph_biconical_base_72_2.5_0.4_custom/outputs" # Make sure this points to your MPXI>=1 folder
    TENSOR = "../../../data/scanner_layouts/hybrid_sc_spect_mph_biconical_base_72_2.5_0.4_custom.tensor"
    ...
    pure_msi_map, swsi_map, max_sens = analyze_multiplexing_ambiguity(
        ...
        min_mpxi=MIN_MPXI # Passes the global config down
    )
    
    visualize_map(
        data_2d=pure_msi_map, source_points_mm=PHANTOM, out_dir=DATA_DIR, 
        title_prefix="Pure MSI Map (No PPDF)", and_cond=AND_CONDITION, 
    ...
    visualize_map(
        ...
        title_prefix="SWSI Map (PPDF Integrated)", and_cond=AND_CONDITION, 
